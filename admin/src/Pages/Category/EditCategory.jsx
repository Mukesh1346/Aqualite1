      ...
      try {
        ...
        if (response?.status === 200) {
          setFormData({
            ...
            status: response?.data?.data?.isCollection || false,
            oldImage: response?.data?.data?.categoryImage || null,
            image: null,
          });
        }
      } catch (error) {
...
if(!fileLimit(formData?.image)) return;
    ...
    try {
      const response = await axiosInstance.put(`/api/v1/category/update-category/${id}`, payload,{
        headers: {
